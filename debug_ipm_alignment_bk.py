...
python debug_ipm_alignment.py   --data_root /home/zhimiao/code_space/kitti360_camera_pose   --drive 2013_05_28_drive_0003_sync   --frame 240   --camera image_02   --out_dir debug_output   --yaw -20   --calib_yaw_fix 4
...
def main():
    ...
    parser.add_argument("--camera", type=str, default="image_02", choices=["image_02", "image_03"])
    parser.add_argument("--yaw", type=float, default=0.0, help="Rotation angle relative to fisheye optical axis")
    ...
    # --- 3. 生成虚拟视角 ---
    xi, K_f, D_f = load_mei_yaml(seq_dir / "calibration" / f"{args.camera}.yaml")
    K_v = make_newK(640, 256, args.hfov).to(device)
    ...
    map1, map2 = cv2.omnidir.initUndistortRectifyMap(
        ...
        K_v.cpu().numpy().astype(np.float64), (640, 256), cv2.CV_32FC1, cv2.omnidir.RECTIFY_PERSPECTIVE
    )
    view_virtual = cv2.remap(img_bgr, map1, map2, cv2.INTER_LINEAR)
# --- Debug: check yaw/rotation in the *fisheye/virtual camera* frame (not world) ---
    ...
    # --- 4. 生成 IPM ---
    sat_path = seq_dir / "satellite_bk" / f"{frame_str}.png"
    sat_img = cv2.resize(cv2.imread(str(sat_path)), (512, 512))
    ...
    fig.suptitle(
        f"IPM Alignment Debug | drive={args.drive} frame={frame_str} cam={args.camera} yaw={args.yaw:.1f} res={args.res}",
        fontsize=12,
    )
    ...
    # (2) Virtual perspective image
    axes[1].imshow(cv2.cvtColor(view_virtual, cv2.COLOR_BGR2RGB))
    axes[1].set_title(f"(2) Virtual view {640}x{256} hfov={args.hfov:.1f}")
    axes[1].axis("off")
