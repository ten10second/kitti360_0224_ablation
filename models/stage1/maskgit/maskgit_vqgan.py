...
class VectorQuantizer(nn.Module):
    ...
    def get_codebook_entry(self, indices):
        # indices are expected to be of shape (batch, num_tokens) or (batch, height, width)
        # get quantized latent vectors
        if len(indices.shape) == 2:
            # 尝试处理非平方形状的 tokens
            batch, num_tokens = indices.shape

            # 检查是否是训练时的形状 (16x40=640 tokens)
            if num_tokens == 640:
                z_q = self.embedding(indices)
                z_q = z_q.reshape(batch, 16, 40, -1).permute(0, 3, 1, 2)
            else:
                # 对于其他形状，尝试使用平方形状（保持向后兼容性）
                try:
                    side = int(math.sqrt(num_tokens))
                    if side * side == num_tokens:
                        z_q = self.embedding(indices)
                        z_q = z_q.reshape(batch, side, side, -1).permute(0, 3, 1, 2)
                    else:
                        raise NotImplementedError(f"Unsupported number of tokens: {num_tokens}")
                except:
                    print(f"Unsupported number of tokens: {num_tokens}")
                    raise
