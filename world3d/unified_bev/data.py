...
def load_frame_records(
    ...
    *,
    front_crop_width: int = FRONT_CROP_WIDTH,
    front_crop_overlap: int = FRONT_CROP_OVERLAP,
    image_size: Tuple[int, int] = (160, 96),
    use_fisheye: bool = True,
    fisheye_yaws_deg: Tuple[float, ...] = (-45.0, 0.0, 45.0),
    rigs: Optional[Dict[str, "FisheyeVirtualRig"]] = None,
) -> Dict[str, List[FrameRecord]]:
    """Load KITTI-360 frame records without building a target-centered sample list."""
    ...
    for drive, rows in raw.items():
        ...
        front_crop_starts = centered_two_crop_starts(
            raw_size[0], front_crop_width, front_crop_overlap, float(K0[0, 2]),
        )
        T_cam_velo = _read_cam_to_velo(d / "calibration" / "calib_cam_to_velo.txt")
        if use_fisheye:
            rigs[drive] = FisheyeVirtualRig(
                d / "calibration", image_size, yaws_deg=fisheye_yaws_deg,
            )
        records: List[FrameRecord] = []
        for row in rows:
            fid = int(row["frame_index"])
            lidar = Path(lidar_root) / drive / "velodyne_points" / "data" / f"{fid:010d}.bin"
            T_imu = poses.get(fid)
...
class UnifiedBEVDataset(Dataset):
    ...
    def _load_records(self, drive_filter: Optional[str]) -> Dict[str, List[FrameRecord]]:
        return load_frame_records(
            self.manifest, self.lidar_root, drive_filter,
            front_crop_width=self.front_crop_width,
            front_crop_overlap=self.front_crop_overlap,
            image_size=self.image_size,
            use_fisheye=self.use_fisheye,
            fisheye_yaws_deg=self.fisheye_yaws_deg,
            rigs=self._rigs,
        )
