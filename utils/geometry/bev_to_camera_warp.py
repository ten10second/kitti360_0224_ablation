...
def warp_bev_to_camera_with_coords(
    ...
    v_sat = H_sat / 2 - Y_off / resolution
    ...
    # 采样（可选：仅需要坐标时可关闭，避免额外开销）
    if return_warped_sat:
        warped_sat = F.grid_sample(
            sat_image, grid, mode='bilinear', padding_mode='zeros', align_corners=True
        )
    else:
        warped_sat = torch.zeros((B, C, cam_height, cam_width), device=device, dtype=sat_image.dtype)
...
def warp_bev_to_camera_with_coords_fisheye_map(
    sat_image: torch.Tensor,
    map1: torch.Tensor,
    map2: torch.Tensor,
    xi: float,
    K_fisheye: torch.Tensor,
    D_fisheye: torch.Tensor,
    T_cam_to_world: torch.Tensor,
    cam_height: int,
    cam_width: int,
    sat_size: int = 512,
    ...
    if B != 1:
        raise ValueError("warp_bev_to_camera_with_coords_fisheye_map currently supports B==1")
    ...
    # Use camera's own position as the center for the BEV patch
    sat_center = t_cam_to_world[:, :2, 0]

    if map1.shape[-2:] != (cam_height, cam_width) or map2.shape[-2:] != (cam_height, cam_width):
        raise ValueError(
            f"map1/map2 shape must be ({cam_height},{cam_width}); got {tuple(map1.shape[-2:])}/{tuple(map2.shape[-2:])}"
        )

    # map tensors expected in raw-fisheye pixel coords (x,y)
    u_raw = map1.to(device=device, dtype=torch.float32).reshape(1, -1)  # (1,N)
    v_raw = map2.to(device=device, dtype=torch.float32).reshape(1, -1)  # (1,N)

    # MEI unprojection (no tangential/radial distortion inversion; assumes map already accounts for D)
    if K_fisheye.dim() == 2:
        K_fisheye = K_fisheye.unsqueeze(0)
    fx = K_fisheye[:, 0, 0:1]
    fy = K_fisheye[:, 1, 1:2]
    cx = K_fisheye[:, 0, 2:3]
    cy = K_fisheye[:, 1, 2:3]
    ...
    xi_t = torch.tensor(float(xi), device=device, dtype=torch.float32).view(1, 1)
    sqrt_term = torch.sqrt(1.0 + (1.0 - xi_t * xi_t) * r2)
    denom = xi_t + sqrt_term

    x = mx / denom
    y = my / denom
    z = (1.0 - xi_t * r2) / denom

    rays_cam = torch.stack([x, y, z], dim=1)  # (1,3,N)

    # normalize for numeric stability
    rays_cam = rays_cam / (torch.norm(rays_cam, dim=1, keepdim=True) + 1e-9)

    rays_world = torch.bmm(R_cam_to_world, rays_cam)

    # Intersect with world ground plane Z = ground_height
    rays_z = rays_world[:, 2:3, :]
    t = torch.where(
        rays_z.abs() > 1e-6,
        (ground_height - t_cam_to_world[:, 2:3, :]) / rays_z,
    ...
    offset_x = X_world - sat_center[:, 0:1]
    offset_y = Y_world - sat_center[:, 1:2]

    u_sat = W_sat / 2 + offset_x / resolution
    v_sat = H_sat / 2 - offset_y / resolution

    u_sat = u_sat.reshape(B, cam_height, cam_width)
    v_sat = v_sat.reshape(B, cam_height, cam_width)
    t_img = t.reshape(B, cam_height, cam_width)

    # valid mask: in front & within bounds & below horizon (reuse existing helper)
    valid_mask = ipm_valid_mask(
        u_sat=u_sat,
        v_sat=v_sat,
        t_img=t_img,
        rays_z=rays_z,
        vv_cam=torch.arange(cam_height, dtype=torch.float32, device=device).view(-1, 1).expand(cam_height, cam_width),
        K=K_fisheye if K_fisheye.dim() == 3 else K_fisheye.unsqueeze(0),
        W_sat=W_sat,
        H_sat=H_sat,
        # This is the main fix: the K used here is for the raw fisheye image, whose cy is not
        # compatible with the virtual camera's height. This causes the ground_pixel check to fail
        # for all pixels. Relaxing it makes the IPM appear.
        cy_margin_px=-1e9,
    )
    ...
    warped_sat = F.grid_sample(sat_image, grid, mode='bilinear', padding_mode='zeros', align_corners=True)
