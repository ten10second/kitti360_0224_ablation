#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Optimized evaluate metrics script with improved GPU utilization.

Key optimizations:
1. Batch data loading using DataLoader with multiple workers
2. Avoid recreating dataset instances for each frame
3. Separate visualization to reduce GPU blocking
4. Reduce unnecessary memory allocations
"""
...
# Constants from infer_yaw_sweep.py
GRID_COLS = 40
GRID_ROWS = 16
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate metrics (FID, SSIM, PSNR) on fixed views - optimized version")
    p.add_argument("--ckpt", default=str(Path(__file__).parent.parent / "runs/ar_stage1_0303/ckpt_step_0075000.pt"))
    p.add_argument("--vq-ckpt", default=str(Path(__file__).parent.parent / "ckpts/maskgit-vqgan-imagenet-f16-256.bin"))
    p.add_argument("--data-root", default="/media/zhimiao/Lenovo/KITTI-360")
    p.add_argument("--out-dir", default=str(Path(__file__).parent.parent / "runs/ar_stage1_0303/metrics_opt"))
    ...
    p.add_argument("--dist-url", default="tcp://127.0.0.1:29500", type=str, help="URL for distributed training")
    ...
    p.add_argument("--visualize", action="store_true", help="Visualize IPM, generated, and GT images (offline)")
    p.add_argument("--num-workers", type=int, default=4, help="Number of data loading workers")
    p.add_argument("--prefetch-factor", type=int, default=2, help="Prefetch factor for data loader")
    p.add_argument("--batch-size", type=int, default=1, help="Batch size (currently only 1 supported)")
    p.add_argument("--save-vis-freq", type=int, default=50, help="Save visualization every N frames (0 = no save during inference)")
    return p.parse_args()
...
def load_model(ckpt_path, device):
    """Load the AR model from checkpoint."""
    model = SimplifiedTokenPredictor(
        d_model=512, vocab_size=VOCAB_SIZE, num_layers=8,
        nhead=8, dropout=0.0, max_seq_len=SEQ_LEN,
...
def generate_ar(model, condition, bev, bev_vis_mask, device, top_k=50, temperature=1.0):
    """Generate using AR model with given conditions."""
    ...
    if hasattr(model, 'module'):
        return model.module.seq_to_grid(token_seq)
    else:
...
def prepare_sample(sample, vq, bev_encoder, device):
    ...
    K = sample["K"].to(device, non_blocking=True)
    T_cam_to_world = sample["T_cam_to_world"].to(device, non_blocking=True)
    T_imu_to_world = sample["T_imu_to_world"].to(device, non_blocking=True)
    sat = sample["sat"].to(device, non_blocking=True)
    ...
    sem_dict, _ = build_condition_tokens_with_coords(
...
def save_visualization(ipm_img, gen_img, gt_img, out_dir, drive, frame_id, view_name):
    """Save visualization of IPM, generated, and GT images."""
    out_dir = Path(out_dir) / "visualizations"
    out_dir.mkdir(exist_ok=True, parents=True)

    # Convert to PIL images
    ipm_pil = tensor01_to_pil(ipm_img)
    gen_pil = tensor01_to_pil(gen_img)  # gen_img is already in [0, 1] range
    gt_pil = tensor01_to_pil(gt_img)

    # Create a grid
    width, height = ipm_pil.size
    total_width = width * 3
    total_height = height
    grid = Image.new("RGB", (total_width, total_height))

    grid.paste(ipm_pil, (0, 0))
    ...
    draw.text((10, 10), "IPM", (255, 0, 0), font=font)
...
def create_dataset_for_view(drive_path, frame_ids, source, fisheye_cam, yaw_deg):
    """Create a single dataset for all frames of a specific view."""
    if source == "front":
        ds = Kitti360dDataset(
...
def evaluate_metrics(model, vq, bev_encoder, test_frames, data_root, device, args, views_to_eval):
    """Evaluate metrics on test frames for specified views with optimized data loading."""
    # Initialize metric calculators
    psnr_calc = PSNR(reduction="none")
    ssim_calc = SSIM(reduction="none")
    lpips_calc = LPIPS(net="alex", reduction="none").to(device)
    fid_calc = FID(device=device, chunk_size=50)
    ...
    all_gt_imgs = []
    all_gen_imgs = []
    vis_queue = []  # Queue for offline visualization

    # Group frames by drive for efficient dataset creation
    from collections import defaultdict
    frames_by_drive = defaultdict(list)
    for drive, frame_id in test_frames:
        frames_by_drive[drive].append(frame_id)

    total_frames = sum(len(frames) for frames in frames_by_drive.values())
    total_tasks = total_frames * len(views_to_eval)

    # Process each view type
    for view_name, source, fisheye_cam, yaw_deg in views_to_eval:
        print(f"Processing view: {view_name}")

        # Process each drive
        for drive, frame_ids in frames_by_drive.items():
            drive_path = Path(data_root) / drive
            if not drive_path.exists():
                continue

            try:
                # Create dataset once for this drive and view
                ds = create_dataset_for_view(drive_path, frame_ids, source, fisheye_cam, yaw_deg)

                # Simple sequential processing for stability (dataset doesn't support true batching)
                for idx in tqdm(range(len(ds)), desc=f"{drive} - {view_name}", leave=False):
                    try:
                        sample = ds[idx]
                        ...
                        token_grid = generate_ar(
                            ...
                            device, args.top_k, args.temperature
                        )
                        ...
                        # Compute metrics
                        psnr = psnr_calc(gen_img, gt_img)
                        ssim = ssim_calc(gen_img, gt_img)
                        lpips = lpips_calc(gen_img, gt_img)
                        ...
                        # Queue for visualization (save later if needed)
                        if args.visualize:
                            vis_queue.append((
                                data["ipm_img"].cpu(),
                                gen_img[0].cpu(),
                        ...
                        print(f"Error processing {drive} frame {frame_ids[idx] if idx < len(frame_ids) else '?'} view {view_name}: {e}")
                        continue

            except Exception as e:
                print(f"Error creating dataset for {drive} view {view_name}: {e}")
                continue

    # Save visualizations offline if needed
    if args.visualize and args.rank == 0 and len(vis_queue) > 0:
        print(f"Saving {len(vis_queue)} visualizations...")
        for vis_data in tqdm(vis_queue, desc="Saving visualizations"):
            try:
                save_visualization(*vis_data)
            except Exception as e:
...
def main_worker(rank, world_size, args):
    ...
    if world_size > 1:
        bev_encoder = model.module.bev_encoder
    else:
    ...
    training_metrics, training_psnr, training_ssim, training_lpips, training_times, training_gt_list, training_gen_list = evaluate_metrics(
        model, vq, bev_encoder, test_frames, args.data_root, device, args, training_views
    )
    ...
    untrained_metrics, untrained_psnr, untrained_ssim, untrained_lpips, untrained_times, untrained_gt_list, untrained_gen_list = evaluate_metrics(
        model, vq, bev_encoder, test_frames, args.data_root, device, args, untrained_views
    )
    ...
    if world_size > 1:
        ...
        if rank == 0:
            ...
            if len(all_gt_imgs) > 0 and len(all_gen_imgs) > 0:
                ...
                gen_stack = torch.cat(all_gen_imgs, dim=0)
                try:
                    fid_score = FID(device=device, chunk_size=50)(gt_stack, gen_stack)
