#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Evaluate metrics (FID, SSIM, PSNR) on fixed views using GPU acceleration with multi-card support."""
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate metrics (FID, SSIM, PSNR) on fixed views")
    ...
    p.add_argument("--out-dir", default=str(Path(__file__).parent.parent / "runs/ar_stage1_0303/metrics"))
    ...
    p.add_argument("--visualize", action="store_true", help="Visualize IPM, generated, and GT images")
    return p.parse_args()
        ...
        for view_name, source, fisheye_cam, yaw_deg in eval_views:
            try:
                ...
                if args.visualize and args.rank == 0:
                    save_visualization(data["ipm_img"], gen_img[0], gt_img[0], args.out_dir, drive, frame_id, view_name)
